...
import com.example.webtoonproject.databinding.FragmentWebviewBinding

class WebViewFragment(private val position : Int, private  val webViewUrl : String) : Fragment() {

    var listner : OnTabLayoutNameChanged? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.webView.webViewClient = WebtoonWebViewClient(binding.progressBar, {
            url -> activity?.getSharedPreferences(SHARED_PREFERENCE, Context.MODE_PRIVATE)?.edit {
        ...
        binding.changeTabNameButton.setOnClickListener {
            ...
            dialog.setPositiveButton("저장") { _, _ ->
                activity?.getSharedPreferences(SHARED_PREFERENCE, Context.MODE_PRIVATE)?.edit(){
                    ...
                    listner?.nameChanged(position, editText.text.toString())
                }
            }
            dialog.setNegativeButton("취소") { dialogInterface, _ ->
                dialogInterface.cancel()
                Toast.makeText(context,"취소되었다능",Toast.LENGTH_SHORT).show()
            }
            dialog.show()
        }
    }
